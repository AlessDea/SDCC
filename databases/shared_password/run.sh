...
docker run --name ${container_name} -e MYSQL_ALLOW_EMPTY_PASSWORD=yes -p ${host_port}:${container_port} -d -v shared-password-db:/var/lib/mysql --network net1 shared-password-db:latest