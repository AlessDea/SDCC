...
docker run --name ${container_name} -e MYSQL_ALLOW_EMPTY_PASSWORD=yes -p ${host_port}:${container_port} -d -v groups-db:/var/lib/mysql --network net-test groups-db:latest