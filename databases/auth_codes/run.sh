...
docker run --name ${container_name} -e MYSQL_ALLOW_EMPTY_PASSWORD=yes -p ${host_port}:${container_port} -d -v auth-codes-db:/var/lib/mysql --network net1 auth-codes-db:latest