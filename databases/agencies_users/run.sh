...
docker run --name ${container_name} -e MYSQL_ALLOW_EMPTY_PASSWORD=yes -p ${host_port}:${container_port} -d -v agencies-users-db:/var/lib/mysql --network net1 agencies-users-db:latest